...
int main(){
	...
	*/
	...
	/* 4: Fill out the server socket configurations i.e. the 
	Port number the socket will be bind to  and the connection 
	interface the socket will connect to, and the Address Family assigned to 
	the socket. 
	*/
	...
	// 9: The following code will only execute if a client has requested to connect with the server created above.
	...
	// 10: Read the clients sent information and store it in the buffer 
	...
	// 12: close the established connection between the client and the server
	close(client); 
