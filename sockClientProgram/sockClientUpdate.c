...
int main(){
	...
	// 1: Create a place holder for server socket
	int serverSock = 0; // this is linux specific, on windows OS, server/client == SOCKET type

	// 2: Create sockaddr_in structure to hold the server specific information
	...
	// 4: Fill out the sockaddr_in with information regarding the server you're connecting to
	...
	// 5: Send connection to the server which has the configuration as set above
	...
	// 6: Message to send to the server assigned to port 5555
	...
	// 8: close connection to the server
	close(serverSock);
